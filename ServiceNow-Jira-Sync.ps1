...
if ($CurrentDate -ge $9Hour -and $CurrentDate -le $18Hour -and $CurrentDayStatus -ne 1) {
    ...
    function Send-Notification($subject, $body, $RecipientList, [array]$Attachment) {
        $username = "UserName"
        $password = Get-Content "C:\Scripts\pwd\$username.txt" | ConvertTo-SecureString
...
"@
        ...
        $Assignee = ($JiraSnowUsers | Where { $_.UserName -eq ($SubTasks | Where { $_.short_description.display_value -eq "Final full backup of server (for archive)" -or $_.short_description.display_value -eq "Server deployment" }).assigned_to.display_value }).JiraID
        if (!$Assignee) {
...
'@ -f $Boundary, $FileName, $FileEnc
    ...
    # Set the credentials
    $User = [User]
    $pass = Get-Content "C:\Scripts\pwd\default\$User.txt" | ConvertTo-SecureString
    ...
    $Pair = "[UserNameMailbox]:$JiraApiToken"
    ...
    function Get-JiraIssues ($StartAt) {
        $Params = @{
            uri = "$JiraInstance/search?jql=project=$ProjectKey and issuetype!=Epic and created>=-30d&fields=key,summary,assignee,status,subtasks,comment,attachment&startAt=$StartAt&maxResults=100"
            headers = @{ "Authorization" = "Basic $Base64" }
            method = "Get"
        }

        $Issues = Try-Expression -expression 'Invoke-RestMethod @Params -UseBasicParsing' -message "Getting all issues in project for the last 30 days starting at $StartAt..."
        return $Issues
    }
...
"@
    ...
    foreach ($User in ($JiraSnowUsers | Where { $_.UserName -ne "Kirill Popov" })) {
        $Params = @{
            Headers = $GlobalHeaders
            Method = "GET"
            Uri = "$ServiceNowInstance/api/now/table/change_request?sysparm_query=assignment_group=dc1031a81b89249081d3c84b1d4bcb63^assigned_to=$( $User.SnowID )^active=true^stateNOT IN-5,-4,0^start_date>javascript:gs.dateGenerate(`'$MonthAgo`','00:00:00')&sysparm_display_value=All"
        }

        $Changes = ""
        $Changes = Try-Expression -expression 'Invoke-RestMethod @params -UseBasicParsing' -message "Getting changes opened for the last 30 days in snow for user `"$( $User.UserName )`"..."
    ...
    foreach ($Item in $RitmTemplatesList) {
        $Params = @{
            Headers = $GlobalHeaders
            Method = "GET"
            Uri = "$ServiceNowInstance/api/now/table/sc_req_item?sysparm_query=cat_item=$( $Item.Id )^active=true^opened_at>javascript:gs.dateGenerate(`'$MonthAgo`','00:00:00')&sysparm_display_value=All"
        }
